...
/**
 * Session Bean implementation class JPAProjectService
 */
@Stateless
@Remote(ProjectServiceRemote.class)
@LocalBean
@TransactionManagement(TransactionManagementType.CONTAINER)
public class JPAProjectService implements ProjectServiceRemote {
	...
	@EJB
	private JPAProfileService profileService;
	...
	public Project createProject(Project pr, ClientProfile cp, int daysToPost)
		...
		Project projectPersisted = em.merge(pr);
		cp.addProject(projectPersisted);
	...
	public Project findProjectByID(int projectID) throws BusinessException {
		...
		if (pr != null) {
			...
			int diffHours = Hours.hoursBetween(actualDate, expiryDate)
					.getHours();
			System.out.println("============ DIFFERENZA DI ORE: " + diffHours);
			if (diffHours > 0) {
				if (diffHours >= 24) {
					daysLeft = Days.daysBetween(actualDate, expiryDate)
							.getDays();
					hoursLeft = diffHours - (daysLeft * 24);
					/*
					 * daysLeft = diffHoursTotal / 24 ; hoursLeft =
					 * diffHoursTotal % 24;
					 */
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void projectUpdate(Project pr, int daysToPost, ClientProfile cp)
			throws BusinessException {
		Date actualDate = new Date();
		Calendar c = Calendar.getInstance();
		c.setTime(actualDate);
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void createProposal(Proposal proposal, int projectID,
			int freelanceUserID) throws BusinessException {

		FreelanceProfile aspirantFreelance = em.find(User.class,
				freelanceUserID).getFreelanceProfile();
		...
		proposal.setRefProject(em.merge(project));
		proposal.setProposalDate(new Date());
		proposal.setStatus(ProposalStatus.NOT_HIRED);
		em.persist(proposal);
		inboxService.sendSystemMessage(SystemMessageSubject.NEW_JOB_PROPOSAL,
				proposal);
	...
	@Override
	public FilterDataResponse<Proposal> findAllProposalsFiltered(
		...
		String queryString = "SELECT p FROM Proposal p WHERE p.refProject.projectID = '"
				+ filterDataRequest.getProjectID()
				+ "'"
				+ " ORDER BY p.proposalDate " + filterDataRequest.getDirSort();
		TypedQuery<Proposal> typedQuery = em.createQuery(queryString,
				Proposal.class);
		TypedQuery<Proposal> typedQueryPaginated = em
				.createQuery(queryString, Proposal.class)
	...
	@Override
	public boolean checkAlreadyPostProposal(User user, Project project)
			throws BusinessException {
		if (user.getFreelanceProfile() != null) {
			String search = "SELECT p FROM Proposal p WHERE p.refProject.projectID = '"
					+ project.getProjectID()
					+ "' AND p.aspirantFreelance.freelanceID = '"
					+ user.getFreelanceProfile().getFreelanceID() + "'";
	...
	// @Override
	// @TransactionAttribute(TransactionAttributeType.REQUIRED)
	// public int createProposalQuestionMessage(Message message, int proposalID)
	// throws BusinessException{
	// Proposal proposal = em.find(Proposal.class, proposalID);
	// em.refresh(proposal);
	// inboxService.incrementNewMessageCounter(proposal,
	// proposal.getRefProject().getClientOwner().getUser().getUserID());
	...
	@Override
	public Proposal findProposalByID(int proposalID, User userLogged)
			throws BusinessException {
		Proposal proposal = em.find(Proposal.class, proposalID);
		if (proposal != null) {
			if (proposal.getAspirantFreelance().getUser().getUserID() == userLogged
					.getUserID()
					| proposal.getRefProject().getClientOwner().getUser()
							.getUserID() == userLogged.getUserID()) {
				return proposal;
			} else {
	...
	public FilterDataResponse<Project> findAllProjectsFiltered(
		...
		int firstResult = filterDataRequest.getItemsForPage()
				* filterDataRequest.getPage()
				- filterDataRequest.getItemsForPage();
		int maxResult = filterDataRequest.getItemsForPage();
		String baseQuery = "SELECT p FROM Project p WHERE p.status LIKE '"
				+ filterDataRequest.getStatus() + "' ";

		String queryUser = "";
		if (filterDataRequest.getUserID() != 0) {
			queryUser = "AND p.clientOwner.user.userID <> "
					+ filterDataRequest.getUserID() + " ";
		}
		...
		if (!"".equals(filterDataRequest.getCategory())) {
			queryCategory = "AND p.projectSubCategory.parentCategory.catID = '"
					+ filterDataRequest.getCategory() + "' ";
		}

		String querySkill = "";
		if (!"".equals(filterDataRequest.getSkill().getName())) {
			String query = "SELECT s FROM Skill s WHERE s.name LIKE '"
					+ filterDataRequest.getSkill().getName() + "'";
		...
		if (!"".equals(filterDataRequest.getItemSort())) {
			...
			order = "ORDER BY " + itemSort + " "
					+ filterDataRequest.getDirSort();
		}
		String query = baseQuery + queryUser + querySearch + queryCategory
				+ querySkill + order;
		System.out.println(query);
		TypedQuery<Project> typedQuery = em.createQuery(query, Project.class);
		TypedQuery<Project> typedQueryPaginated = em
				.createQuery(query, Project.class).setFirstResult(firstResult)
				.setMaxResults(maxResult);
		if (!"".equals(filterDataRequest.getSkill().getName())) {
			typedQuery.setParameter("skill", filterDataRequest.getSkill());
			typedQueryPaginated.setParameter("skill",
					filterDataRequest.getSkill());
		}
		int totalItems = typedQuery.getResultList().size();
	...
	public FilterDataResponse<Project> findAllClientProjectsFiltered(
		...
		if (filterDataRequest.getStatus().equals("COMPLETED")) {
			baseQuery += "SELECT p FROM Proposal prop JOIN prop.refProject p WHERE prop.status LIKE '"
					+ filterDataRequest.getStatus() + "' ";
		} else {
			if (filterDataRequest.getStatus().equals("HIRING_CLOSED")) {
				...
				baseQuery += "SELECT p FROM Project p WHERE p.status LIKE '"
						+ filterDataRequest.getStatus() + "' ";
			}
		}
		String queryUser = "";
	...
	public FilterDataResponse<Project> findAllFreelanceProjectsFiltered(
		...
		if (status.equals("watch")) {
			baseQuery += "SELECT p FROM FreelanceProfile fp JOIN fp.projectWatchList p WHERE fp.user.userID = "
					+ filterDataRequest.getUserID() + " ";
		} else {
			queryUser += "AND prop.aspirantFreelance.user.userID = "
					+ filterDataRequest.getUserID() + " ";
			baseQuery = "SELECT p FROM Proposal prop JOIN prop.refProject p WHERE prop.status LIKE '"
					+ filterDataRequest.getStatus() + "' ";
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void addFeedback(FeedBack feedback, int projectlID)
			throws BusinessException {
		Proposal p;
		// System.out.println(feedback.getType());
		if (feedback.getType().equals("client")) {
			String queryString = "SELECT p FROM Proposal p "
					+ "WHERE p.refProject.projectID="
					+ feedback.getJobEvaluated().getRefProject().getProjectID()
					+ " AND p.status LIKE 'WORKING'";
			TypedQuery<Proposal> query = em.createQuery(queryString,
					Proposal.class);
			...
			System.out.println("Progetti prima: " + totalProjects_old
					+ " Progetti dopo: " + totalProjects);
	...
	public ResponseGrid<Project> findAllProjectPaginated(RequestGrid requestGrid)
		...
		String search = baseSearch + " ORDER BY p." + requestGrid.getSortCol()
		...
		TypedQuery<Project> query = em.createQuery(search, Project.class)
				.setFirstResult(requestGrid.getiDisplayStart())
				.setMaxResults(requestGrid.getiDisplayLength());
		List<Project> projects = query.getResultList();

		return new ResponseGrid<Project>(requestGrid.getsEcho(), iTotalRecords,
				iTotalRecords, projects);
